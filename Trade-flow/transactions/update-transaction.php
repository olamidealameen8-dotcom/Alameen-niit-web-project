...
if ($transactionsId == '') {
    $response = [
        'success' => false,
        'message' => "TRANSACTION ID IS REQUIRED, Kindly fill in the transaction ID to continue"
    ];
    goto end;
}
...
if (mysqli_num_rows($checkPaymentMethodQuery) == 0) {
    $response = [
        'success' => false,
        'message' => "INVALID PAYMENT METHOD ID! Selected payment method ID ($paymentMethodId) does not exist in payment_method_tab"
    ];
    goto end;
}
...
mysqli_query($conn, "UPDATE transactions_tab SET customer_id = '$customerId', price = '$price', payment_method_id = '$paymentMethodId', status_id = '$statusId', updated_at = NOW() WHERE transactions_id = '$transactionsId'") or die(mysqli_error($conn));
...
$transactionData = mysqli_fetch_assoc($fetchUpdatedTransaction);
